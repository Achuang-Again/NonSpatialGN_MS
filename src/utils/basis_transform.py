...
import mindspore_gl
from mindspore_gl import dataset as ds
# import dglFunction
...
def check_edge_max_equal_num_nodes(g):
    edge_idx = ms.ops.stack([g.edges()[0], g.edges()[1]], dim=0)
...
def check_repeated_edges(g):
    edge_idx = ms.ops.stack([g.edges()[0], g.edges()[1]], dim=0)
    edge_unique = ms.ops.unique(edge_idx, dim=1)
...
def basis_transform(g,
    ...
    if basis == 'rho':
        ...
        graph_matrix = ms.ops.matmul(eig_vec, ms.ops.matmul(eig_val_smoothed.diag_embed(), eig_vec.transpose(-2, -1)))
    elif basis == 'eps':
        deg = adj.sum(1)
    ...
    full_one = ms.ops.ones_like(graph_matrix, dtype=graph_matrix.dtype).nonzero(as_tuple=True)
    # print(full_one)
